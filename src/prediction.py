...
def __snap_and_interpolate(df: pl.DataFrame) -> pl.DataFrame:
    """
    Snap datetime to 5-minute intervals and interpolate missing data.
    
    Args:
        df: DataFrame with columns ['location', 'lmp', 'datetime']
    
    Returns:
        DataFrame with snapped datetimes and interpolated values
    """
    ...
    df_agg = df_snapped.group_by(["location", "datetime_snapped"]).agg(
        pl.col("lmp").mean()
    ...
    result = (
        complete_grid
        .join(df_agg, on=["location", "datetime"], how="left")
        .sort(["location", "datetime"])
        .with_columns(
            pl.col("lmp").interpolate().over("location")
        )
    )
...
def generate_predictions():
    ...
    pred_df = pipeline.predict_df(
        ...
        quantile_levels=[0.1, 0.3, 0.5, 0.7, 0.9],  # Quantile for probabilistic forecast
