...
app.layout = html.Div(style={'backgroundColor': '#0d1117', 'color': '#c9d1d9', 'fontFamily': 'Arial, sans-serif', 'padding': '20px'}, children=[
    html.H1('MISO Market LMP Dashboard', style={'textAlign': 'center', 'color': '#AAF0C1'}),
    html.Div([
        html.Label('Select Location:', style={'fontSize': '18px', 'marginRight': '10px'}),
        dcc.Dropdown(
            id='location-dropdown',
            options=[{'label': loc, 'value': loc} for loc in locations],
            value=locations[0],  # Default to first location
            style={'width': '300px', 'backgroundColor': '#161b22', 'color': '#c9d1d9', 'border': '1px solid #30363d'}
        )
...
def update_graph(selected_location):
    ...
    filtered_df = dashboard_df[dashboard_df['location'] == selected_location].sort_values('datetime')
